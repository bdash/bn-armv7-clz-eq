...
use bn_bdash_extras::llil::{
    Expression, InstrMatch, Instruction, InstructionKind::If, require_full_register,
};
...
fn register_activity(workflow: Option<WorkflowBuilder>) -> Result<(), ()> {
    let Some(workflow) = workflow else {
        log::warn!(
            "Skipping activity registration as core.function.metaAnalysis could not be found"
        );
        return Err(());
    };
...
pub extern "C" fn CorePluginInit() -> bool {
    ...
    register_activity(Workflow::cloned("core.function.metaAnalysis")).is_ok()
}
